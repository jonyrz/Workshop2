CREATE OR REPLACE PACKAGE BODY  video_pkg IS
	PROCEDURE exception_handler(errcode IN NUMBER, contex IN
		VARCHAR2) IS
	BEGIN
		IF errcode= -1 THEN
			RAISE_APPLICATION_ERROR(-20001,'El numero ya fue asignado a otro usario intente con otro');
		ELSIF errcode=-2291 THEN
			RAISE_APPLICATION_ERROR(-20002,contex||'intento usar una FK invalida');
		ELSE
		RAISE_APPLICATION_ERROR(-20999,'Error no controlado in ',contex||' contacta al aldminsitrador con la siguiente informacion: '||CHR(13)||SQLERRM);
		END IF;
	...
	BEGIN
		...
		FOR rented_rec IN rented_crs LOOP
			DBMS_OUTPUT.PUL_LINE('Pelicula reservada hasta: '||rented_rec.exp_ret_date);
 	...
 	BEGIN
 		...
 		UPDATE title_copy
 			...
 			WHERE title_id=titleid AND copy_id=copy_id;
 		FOR rented_crs IN rented_crs LOOP
 			IF rented_crs%FOUND THEN
 				DBMS_OUTPUT.PUL_LINE('Pelicula en espera fue reservada por el usuario: '||res_rec.member_id);
 			END IF;
 		END LOOP;
 	EXCEPTION
 		WHEN OTHERS THEN
 	...
 	END return_movie;
 FUNCTION new_rental(memberid IN rental.memmber_id%TYPE,
...
BEGIN 
	SELECT member_id INTO memberid
		...
		WHERE lower (last_name) = lower (membername);
	FOR copy_rec IN copy_csr LOOP
		IF copy_rec.status = 'DISPNIBLE' THEN
			UPDATE title_copy
				SET status = 'RENTADO'
				WHERE CURRENT OF copy_csr;
			INSERT INTO rental (book_date, copy_id, member_id, title_id,exp_ret_date)
